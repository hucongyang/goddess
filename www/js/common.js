...
(function() {
    ...
    window.JBox = {
        ...
        init: function(config) {
            ...
            delete this.config.okValue;
            delete this.config.cancelValue;
            this.dialog = art.dialog(this.config);
        },
        alert: function(msg, callback, time) {
            this.config.content = msg;
            if (typeof callback == "function") {
                this.config.ok = callback;
            } else {
                this.config.ok = function() {
                };
            }
            if (typeof time != "undefined") {
                this.config.time = time;
            }
            this.config.okValue = "确定";
            this.config.cancelValue = "取消";
            this.dialog = art.dialog(this.config);
        },
     ...
     * 省市联动
     */
    $.fn.CitySelector = function(selected) {
        var _self = $(this);
        ...
        function addProvincesOption() {
            ...
            $.each(_citylist, function(i, n) {
                var _html = "<option value='" + n[0] + "'";
                _html += ">" + n[0] + "</option>";
                $(_html).appendTo($(_province));
            });
        }
            ...
            $.each(_citylist, function(i, n) {
                if (_province.value == n[0]) {
                    ...
                    $.each(_citylist[i], function(j, m) {
                        if (j > 0) {
                            var _html = "<option value='" + m + "'";
                            _html += ">" + m + "</option>";
                            $(_html).appendTo($(_city));
                        }
                    });
                }
            });
        ...
        //初始化已选的对象
        $(_province).val(selected.province).trigger("change");
        $(_city).val(selected.city);
    };
})();
